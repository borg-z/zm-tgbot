...
def get_from_api():
    ...
    last_page = str(result[u'pagination'][u'pageCount'])
    ...
    # last event's id
    last_event_id = result[u'events'][-1][u'Event'][u'Id'].encode("utf-8")
    ...
    prev_page = int(last_page) - (int(last_event_id) -
                                  int(last_event_id_prev)) / per_page_count
    ...
    if int(last_page) == int(prev_page):
        ...
        while int(prev_page) is not int(last_page) + 1:
            ...
            url = urlopen(zm_url + 'api/events.json?page=' +
                          str(prev_page)).read()
            result = json.loads(url)
            for i in result[u'events']:  # event's loop
                id = i[u'Event'][u'Id'].encode("utf-8")
    ...
    for event in last_events_id:  # Now we get data for each event
        ...
        result = json.loads(url)
        # path like "events/1/17/06/01/15/26/12/""
        eventpath = result[u'event'][u'Event'][u'BasePath']
        ...
        # we get frame from middle of frame list
        middle = len(alarm_frame_id) / 2
        try:
            middle_alarm_frame_url = zm_url + eventpath + \
                alarm_frame_id[middle].zfill(5) + '-capture.jpg'
        except (IndexError):
            print 'slow frame'
            time.sleep(3)
            middle_alarm_frame_url = zm_url + eventpath + \
        ...
        out_dict[event] = {'start time': start_time,
                           'score': score, 'link': link, 'camera': monitor_name}
...
def monitoring():
    ...
    for key in result:
        ...
        msg = result[key]
        print("{0}\n{1}\n{2}\n").format('Event Start: ' +
                                        msg['start time'], 'Camera: ' + '#' + msg['camera'], msg['link'])
        print msg, 'msg'
